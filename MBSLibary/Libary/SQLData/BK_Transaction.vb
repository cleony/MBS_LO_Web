...
#End Region
        Public Function GetAllTransaction(ByVal DocType As String) As DataTable
            ...
            Try
                ...
                'sql &= " , Choose (BK_Transaction.DocType,'ฝากเงิน', 'ถอนเงิน','ชำระเงินกู้','คิดดอกเบี้ย','ปิดบัญชี','ปิดบัญชีเงินกู้' ) as Doctype  "
                sql &= " , case when BK_Transaction.DocType = '1' then N'ฝากเงิน' "
                sql &= " when  BK_Transaction.DocType = '2' then N'ถอนเงิน'"
                sql &= " when  BK_Transaction.DocType = '3' then N'ชำระเงินกู้'"
                sql &= " when  BK_Transaction.DocType = '4' then N'คิดดอกเบี้ย'"
                sql &= " when  BK_Transaction.DocType = '5' then N'ปิดบัญชี'"
                ...
                '*********** กรณีที่ยกเลิกให้ดูที่ RefDocNo <> '' เนื่องจากบางอันสถานะยกเลิกยังเป็น 1 อยู่
                'sql &= ", IIF(BK_Transaction.Status = '1',IIF(RefDocNo <> '','ยกเลิก***','ใช้งาน'),'ยกเลิก***') AS Status "
                sql &= " ,  case when BK_Transaction.Status = '1' then (case when RefDocNo <> '' then N'ยกเลิก***' else N'ใช้งาน' end ) "
        ...
        Public Function GetAllTransactionOrderByDocNo(ByVal DocType As String, ByVal Date1 As Date, ByVal Date2 As Date) As DataTable
            ...
            Try
                ...
                If DocType = "'3','6'" Then
                    ...
                    sql &= " ,  case when BK_Transaction.Status = '1' then  N'ใช้งาน'   "
                    sql &= "   else N'ยกเลิก***' end as Status"
                Else
                    sql &= " ,  case when BK_Transaction.Status = '1' then (case when RefDocNo <> '' then N'ยกเลิก***' else N'ใช้งาน' end ) "
                    sql &= "   else N'ยกเลิก***' end as Status"
        ...
        Public Function InsertTransaction(ByRef Info As Entity.BK_Transaction, ByVal RunningFlag As Boolean) As Boolean
            ...
            Try
                ...
                ElseIf Info.DocType = "1" Or Info.DocType = "2" Or Info.DocType = "4" Then
                    ...
                    sql &= " where AccountNo = '" & Share.FormatString(Info.AccountNo) & "' "
                    '======== ต้องเพิ่มกรณียกเลิกไปด้วยกันไปแก้ไขสถานะ ห้ามถอน  ถ้าเป็นเปิดบัญชีอยู่แล้วหรือเป็น ห้ามถอนก็ไม่ต้องไป update 30/05/59
                    sql &= " and Status = '2' "
                    '  sql &= " and BranchId = '" & Share.FormatString(Info.BranchId) & "'"
                    cmd = New SQLData.DBCommand(sqlCon, sql, CommandType.Text)
                    cmd.ExecuteNonQuery()
        ...
        Public Function UpdateStatusTransaction(ByVal DocNo As String, ByVal BranchId As String _
            ...
            Try
                ...
                ' แยกกรณีออกเป็น ยกเลิกกับกรณีปกติ 
                If St = "2" Then
                    '=========== กรณียกเลิกเอกสาร จะต้องยกเลิกเฉพาะข้อมูลที่เป็น ฝาก/ถอน เท่านั้นต้องไม่ไปยกเลิกพวกคิดดอกเบี้ยแล้วตีจ่ายดอกเบี้ยออกด้วย
                    '===== ดึงเอกสารใบเก่าขึ้นมา 
                    ...
                    While i >= 0
                        '======= กรณีเป็นเงินฝากให้ยกเลิกแค่บรรทัดรายการสุดท้ายที่ทำ 

                        If TransactioInfo.DocType = "1" Then
                            '========= กรณีเคสยกเลิก ต้องแยกออกเป็น ฝากประจำกับ
                            ...
                            cmd = New SQLData.DBCommand(sqlCon, sql, CommandType.Text)
                            cmd.ExecuteNonQuery()
                            '===กรณีฝาก ยกเลิกแค่บรรทัดสุดท้ายบรรทัดเดียว
                            Exit While

                        ElseIf TransactioInfo.DocType = "2" Then
                            '========= กรณีถอนให้ sum ยอดถอนจากล่างขึ้นบน(กันกรณีฝากประจำ) พอได้มากกว่าจำนวนเงินใน transaction ให้ออกจาก loop ไป
                            sql = " Update BK_Movement "
                            sql &= " Set StCancel = '1' "
                            sql &= " , RefDocNo = '" & RefDocNo & "'"
                            sql &= " where  DocNo = '" & DocNo & "'"
                        ...
                        ElseIf TransactioInfo.DocType = "5" Then
                            ...
                            '==== เช็ควันถ้าน้อยกว่าก็ไม่ต้องยกเลิกเพราะจะมีกรณีไม่จ่ายดอกเบี้ยปิดบัญชีด้วย
                            ...
                            '== ทำยกเลิกถึงแค่เจอบรรทัดคิดดอกเบี้ยจากล่างขึ้นบนเพราะเป็นดอกเบี้ยปิดบัญชีถ้ายกเลิกการปิดบัญชีต้องยกเลิกคิอดดอกเบี้ยออกไปด้วย
                            If MovementInfos(i).MovementDate.Date < TransactioInfo.MovementDate.Date Then
                                Exit While
                            End If
                        End If
